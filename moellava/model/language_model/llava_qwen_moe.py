...
class RePaMLP(nn.Module):
    def __init__(self, config: QWenConfig):
        ...
        if self.reparamed:
            ...
            if self.gated_ratio == 1.0:
                # All channels are active, no masking and reparam projection needed
                self.w1 = nn.Linear(
                    config.hidden_size, 
                ...
                # All channels are masked, only reparam projection needed
                self.w1 = None
                self.w2 = None
                self.c_proj = None
                self.act_fn = None
            ...
            # Not reparameterized, all channels active initially
            self.w1 = nn.Linear(
                config.hidden_size, 
                self.intermediate_size, 
                bias=not config.no_bias
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.reparamed:
            ...
            if self.repa_proj is not None and self.w1 is not None and self.w2 is not None and self.c_proj is not None:
                return self.c_proj(self.w1(x) * self.act_fn(self.w2(x))) + self.repa_proj(x)
            elif self.repa_proj is not None:
                return self.repa_proj(x)
            else:  
                return self.c_proj(self.w1(x) * self.act_fn(self.w2(x)))
        else:
            # If not reparameterized, track running means and masks
            # Compute gated activation
            x_in = self.w1(x) # [B, E, N, C]
            x_gate = self.act_fn(self.w2(x))  # [B, E, N, C]
            ...
            x = self.c_proj(replaced_gate * x_in)
    ...
    def reparam(self):
        if not self.reparamed:
            ...
            if linear_idx.numel() > 0:
                ...
                repa_proj_weight = self.c_proj.weight[:, linear_idx] @ (self.w1.weight[linear_idx] * self.masked_replacements[linear_idx].unsqueeze(1))
            ...
            if nonlinear_idx.numel() > 0:
                ...
                w1_weight = self.w1.weight[nonlinear_idx]
                self.w1 = nn.Linear(self.hidden_size, nonlinear_idx.numel(), bias=False)
                self.w1.weight = nn.Parameter(w1_weight)
                
                w2_weight = self.w2.weight[nonlinear_idx]
                self.w2 = nn.Linear(self.hidden_size, nonlinear_idx.numel(), bias=False)
                self.w2.weight = nn.Parameter(w2_weight)

                c_proj_weight = self.c_proj.weight[:, nonlinear_idx]
                self.c_proj = nn.Linear(nonlinear_idx.numel(), self.hidden_size, bias=False)
                self.c_proj.weight = nn.Parameter(c_proj_weight)
            else:
                self.w1 = None
                self.w2 = None
                self.c_proj = None
